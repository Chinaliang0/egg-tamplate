...
class UploadController extends Controller {
    ...
    async create() {
        ...
        attachment.filename =
            new Date().getTime() + Math.random().toString(36).substr(2) + path.extname(filename).toLocaleLowerCase()
        // attachment.url = `/uploads/${attachment._id.toString()}${extname}`
        // 组装参数 stream
        const target = path.join('app/public/uploads', `${attachment.filename}`)
        ...
        // 设置响应内容和响应状态码
        ctx.helper.success(ctx, attachment, '创建成功')
    }
    ...
    async creataOss() {
        ...
        attachment.extname = extname
        // attachment.filename = filename
        attachment.filename = new Date().getTime() + '-' + Math.random().toString(36).substr(2) + extname
        // attachment.url = `/uploads/${attachment._id.toString()}${extname}`
        // 组装参数 stream
        const target = `egg-oss-demo/${attachment.filename}`
        // const writeStream = fs.createWriteStream(target)
        // 文件处理，上传到云存储等等
        // const data = await this.ctx.model.Gather.findByPk(id);
        // if (!data) {
        ...
        try {
            ...
            await data.update({ avatar: target })
            await ctx.oss.putStream(target, stream)
            // await awaitWriteStream(stream.pipe(writeStream))
        } catch (err) {
            // 必须将上传的文件流消费掉，要不然浏览器响应会卡死
            await sendToWormhole(stream)
